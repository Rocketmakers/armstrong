...
export class CalendarInput extends React.Component<ICalendarInputProps & IFormInput, ICalendarInputState> {
  ...
  onDaySelected(date: moment.Moment) {
    ...
    this.change(newDate.format(isoFormat));
  }
  ...
  getDayComponent(notInCurrentMonth: boolean, dayClicked: (d: moment.Moment) => void, date: moment.Moment) {
    ...
    const isSelected = d.format(isoFormat) === this.props.data[this.props.prop];
  ...
  propsDateAsMoment(): moment.Moment {
    return moment(this.props.data[this.props.prop], isoFormat, true);
  }
  ...
  render() {
    ...
    return (
      <div className={rootClasses}>
        ...
        {!this.props.alwaysShowCalendar && this.props.data[this.props.prop] && !this.props.disableClear &&
          <div className="clear-date-button" onClick={() => this.change(null)}><Icon icon={Icon.Icomoon.cross} /></div>
        }
