...
export interface IInfinitePagingResult<T> { data: T[], nextPageToken?: PageToken }
...
export function useInfinitePaging<T>(settings: IUseInfinitePagingSettings<T>) {
  const [state, setState] = React.useState<IUseInfinitePagingState<T>>(initialState())
  const [isFetching, setIsFetching] = React.useState(false)

  const fetcher = React.useCallback((currentItems: T[], fetchPageToken?: PageToken) => {
    setIsFetching(true)
    settings.fetch(fetchPageToken).then(v => {
      const noReturnedItems = !v.data || v.data.length === 0
      const items = noReturnedItems ? currentItems : [...currentItems, ...v.data]
      setState({ items, nextPageToken: v.nextPageToken, hasFinished: noReturnedItems || !v.nextPageToken, hasData: true, error: undefined })
      setIsFetching(false)
    }).catch(error => {
