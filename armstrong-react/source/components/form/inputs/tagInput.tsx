...
const TagInputRef: React.ForwardRefRenderFunction<ITagInput, ITagInputProps> = (props, ref) => {
  ...
  const notifyTagsChange = React.useCallback(
    (newTags: string[], newTag: string, change: "add" | "remove" | "set") => {
      setTags(newTags);
      setSuggestions([]);
      if (onTagsChange) {
  ...
  const removeTag = React.useCallback(
    (index: number) => () => {
      ...
      notifyTagsChange(
        utils.array.filter(tags, (__, idx) => idx !== index),
        tagToRemove,
        "remove"
      );
      input.current.focus();
      ...
      return (
        <div className="suggestions">
          {newSuggestions.map((s, i) => (
            <div key={s} className={ClassHelpers.classNames({ selected: i === suggestionIndex })} onClick={addTagCallback(s)}>
              {s}
            </div>
          ))}
        </div>
  ...
  const classes = React.useMemo(() => {
    return ClassHelpers.classNames("armstrong-input", "tag-input", className, {
